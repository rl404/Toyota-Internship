<html>
	...
	<div class="ui container">
		...
		<?php
			if($row['result'] != "O"){
				echo "<div class='ui red inverted center aligned segment'>
						<h3 class='ui header'>
							PE DIDN'T PROCESS THIS PROPOSAL (YET).
							<div class='subheader'>
		...
		<div class='ui equal width grid'>
			...
			<div class='column'>
				<form action="inputupdate.php" method="post">	
					<div class='ui segment'>
						...
						<div class='ui form'>
							...
							<div class='two fields'>
								<div class='field'>
									...
									<input type='text' name='costperpc' value='<?php echo $row['costPerPc']?>'>
								</div>
								<div class='field'>
									<label>Cost Reduction / Vehicle</label>
									<input type='text' name='costreduction' value='<?php echo $row['costReduction']?>'>
								</div>
							</div>
							<div class='two fields'>
								<div class='field'>
								...
								<div class='field'>
									<label>Cost Impact / Year</label>
									<input type='text' name='costimpact' value='<?php echo $row['costImpact']?>'>
								</div>
							</div>
						</div>
					</div>
						...
						<div class='ui form'>
							<div class="required field">
								...
								<select class="ui search dropdown" name="status" required>
									...
									<option value='X'>X: Rejected (No cost effect)</option>
								</select>
							</div>

							<div class='required field'>
								<label>Received Date From PE-ED</label>
								<div class="ui calendar calendarinput">						
								  <div class="ui input left icon">
								    <i class="calendar icon"></i>
								    <?php
								    	if($row['receivedFromPE'] != "0000-00-00"){
								    		echo "<input type='text' name='pp1date' value='$row[receivedFromPE]' required>";
								    	}else{
								    		echo "<input type='text' name='pp1date' placeholder='date' required>";
								    	}
								    ?>
								  </div>
								</div>
							...
							<div class='field'>
								<label>Send Date to PuD</label>
								<div class="ui calendar calendarinput">						
								  <div class="ui input left icon">
								    <i class="calendar icon"></i>
								    <?php
								    	if($row['sendToPuD'] != "0000-00-00"){
								    		echo "<input type='text' name='pp2date' value='$row[sendToPuD]'>";
								    	}else{
								    		echo "<input type='text' name='pp2date' placeholder='date'>";
								    	}
								    ?>
								  </div>
								</div>
							...
							<div class='field'>
								<label>Replied Date From PuD</label>
								<div class="ui calendar calendarinput">						
								  <div class="ui input left icon">
								    <i class="calendar icon"></i>
								    <?php
								    	if($row['repliedFromPuD'] != "0000-00-00"){
								    		echo "<input type='text' name='pp3date' value='$row[repliedFromPuD]'>";
								    	}else{
								    		echo "<input type='text' name='pp3date' placeholder='date'>";
								    	}
								    ?>
								  </div>
								</div>
							...
							<div class='field'>
								<label>Send Date to EA-ED</label>
								<div class="ui calendar calendarinput">						
								  <div class="ui input left icon">
								    <i class="calendar icon"></i>
								    <?php
								    	if($row['sendToEA'] != "0000-00-00"){
								    		echo "<input type='text' name='pp4date' value='$row[sendToEA]'>";
								    	}else{
								    		echo "<input type='text' name='pp4date' placeholder='date'>";
								    	}
								    ?>
								  </div>
								</div>
