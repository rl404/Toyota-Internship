	...
	for($i = 0; $i < count($_POST['job']); $i++){	
		...
		for($j = 1; $j < 32; $j++){			
			...
			if(notempty($_POST['jobhour'][$i][$j])){
				...
				$separatedJob = explode("---",$job);
				
				$selectSql = "SELECT * FROM data 
							WHERE staffName='$_SESSION[username]'
							AND jobName='$separatedJob[0]'
							AND jobDesc='$separatedJob[1]'
				...
				if($selectResult->num_rows > 0){
					while($row = mysqli_fetch_assoc($selectResult)) {
						...
						$updateSql = "UPDATE data SET jobHour=$jobhour,jobName='$separatedJob[0]',jobDesc='$separatedJob[1]',projectCode='$project'
								WHERE id='$row[id]'";
					...
					$insertSql = "INSERT INTO data (staffName,deptCode,jobName,jobDesc,jobDate,jobHour,projectCode) 
					VALUES ('$_SESSION[username]','$_SESSION[deptname]','$separatedJob[0]','$separatedJob[1]','$date','$jobhour','$project')";
