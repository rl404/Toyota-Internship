...
<?php
	...
	if($selectResult->num_rows > 0){
		while($row = mysqli_fetch_assoc($selectResult)) {
			if($jobCurrent != $row['jobName'] || $deptCurrent != $row['deptCode']){
				...
				$selectSql2 = "SELECT * FROM job where jobName='$row[jobName]' and deptCode='$row[deptCode]' order by jobOrder+0";

				$selectResult2 = $conn->query($selectSql2);
				while($row2 = mysqli_fetch_assoc($selectResult2)) {

					$row2['jobDesc'] = str_replace("\n", "<br>", "$row2[jobDesc]"); 
					$row2['jobDesc'] = str_replace(";", "<br>", "$row2[jobDesc]"); 

					if(empty($row2['jobOrder'])){
						$row2['jobOrder'] = "";
					}else{
						$row2['jobOrder'] .= ".";
					}

					$jobdesc .= "$row2[jobOrder] $row2[jobDesc]<br>";
				}
				...
				echo "		$no
						...
						<td>$row[jobName]</td>
						<td>$row[deptCode]</td>
						<td class='left aligned'>$jobdesc</td>
					</tr>";
